...
function applyQuotesLanguage(lang) {
    ...
    multiQuotes.forEach(q => window.quotes.push(q[lang] || q.en));

    const quoteElement = document.querySelector('.quote');
    if (quoteElement && window.quotes.length > 0) {
        quoteElement.textContent = window.quotes[currentQuoteIndex];
    }
}
...
function displayRandomQuote() {
    ...
    let randomIndex;
    do {
        randomIndex = Math.floor(Math.random() * window.quotes.length);
    } while (randomIndex === currentQuoteIndex && window.quotes.length > 1);
